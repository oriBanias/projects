...
typedef void* (*UpdateValueFunction) (void* _originalValue, const void* _updateValue);
...
typedef struct Map_Stats {
	...
	size_t buckets;             /* total number of buckets */ /*bucket is list*/
	size_t chains;              /* none empty buckets (having non zero length list) */ /*list that is not empty*/
